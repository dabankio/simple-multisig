...
contract('SimpleMultiSig', function (accounts) {
  ...
  let createSigs = function (signers, multisigAddr, nonce, destinationAddr, value, data, executor, gasLimit) {
    ...
    // console.log('dbg', 'values', 'dest', destinationAddr, 'exec', executor, 'data', data)
    let txInput = [
      TXTYPE_HASH,
  ...
  let executeSendSuccess = async function (owners, threshold, signers, done) {
    ...
    // Receive funds
    await web3SendTransaction({ from: accounts[0], to: multisig.address, value: web3.utils.toBN(web3.utils.toWei('0.1'), 'ether') })
    ...
    let value = web3.utils.toBN(web3.utils.toWei('0.01'), 'ether')
  ...
  let executeSendFailure = async function (owners, threshold, signers, nonceOffset, executor, gasLimit, done) {
    ...
    // Receive funds
    await web3SendTransaction({ from: accounts[0], to: multisig.address, value: web3.utils.toBN(web3.utils.toWei('2'), 'ether') })

    let randomAddr = web3.utils.sha3(Math.random().toString()).slice(0, 42)
    let value = web3.utils.toBN(web3.utils.toWei('0.1'), 'ether')
